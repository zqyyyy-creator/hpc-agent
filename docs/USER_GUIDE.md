...
* **TUI**：按 `Ctrl+X`、`F10`、`q`，或在输入框输入 `/exit`。
...
```text
/clear      清理当前对话上下文，保留右侧 Job Monitor
/clear all  清理当前对话上下文和右侧 Job Monitor
/exit       退出 TUI
```
...
`/help` 显示通用快捷入口；`/help job` 聚合作业查询、监控、清理、记录归档等常用命令；`/help vasp` 聚合 VASP 输入生成、提交、同步、报告和清理命令。`/clear` 只清理当前对话上下文并保留监控区域；`/clear all` 会同时清理右侧监控区域；`/exit` 退出 TUI。
